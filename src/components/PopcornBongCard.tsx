...
export const PopcornBongCard: React.FC<PopcornBongCardProps> = ({
  ...
  const clientToken = useMemo(() => {
    ...
    if (currentUserName) {
      ...
      if (!tok) {
        tok = `usr_${currentUserName.toLowerCase()}_${Date.now()}_${Math.random().toString(36).substring(2, 6)}`;
    ...
    return (
      <div className="mb-4 pb-4 border-b border-zinc-800 flex flex-wrap items-center justify-between gap-2.5 text-xs">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          {onSelectUser && participants.length > 0 && (
            <div className="flex items-center gap-1.5">
              <ArrowRightLeft className="w-3.5 h-3.5 text-zinc-500" />
              <select
                id={`popcorn-user-select-${variant}`}
                value={currentUserName || ''}
                onChange={(e) => onSelectUser(e.target.value ? e.target.value : null)}
                ...
                title="Bytt bruker for å se eller hente unik popcorn-bong"
              >
                <option value="">Gjest (ikke påmeldt)</option>
                {participants.map((p) => (
                  <option key={p.id} value={p.firstName}>
                    {p.firstName}
                  </option>
                ))}
              </select>
  ...
  return (
    ...
    >
      ...
      <p className="text-zinc-300 text-sm sm:text-base leading-relaxed mb-3">
        {currentUserName ? (
          <>
            Har du lyst på gratis popcorn, <strong className="text-amber-400">{currentUserName}</strong>? Trykk{' '}
        ...
        >
          {loading ? (
            ...
            <>
              ...
              {currentUserName ? `TA MOT POPCORN (${currentUserName})` : 'TA MOT POPCORN'}
            </>
          )}
        </button>
