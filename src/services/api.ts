...
export async function fetchState(): Promise<AppState> {
  try {
    ...
    const cached = typeof window !== 'undefined' ? localStorage.getItem('lillesand_state_cache') : null;
    if (cached) {
      try {
...
export async function simulateTournament(count: 16 | 31): Promise<AppState> {
